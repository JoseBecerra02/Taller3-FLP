...
#|datatype: ambiente
  Propósito: define un tipo de dato llamado ambiente
  <amb> -> vacio
  <amb> -> valor
  <amb> -> ambiente
|#
(define-datatype  ambiente ambiente?
  (vacio)
  (extendido(var symbol?)
            (num number?)
            (amb ambiente?)))


#|Función: extend-env
  Propósito: Recibe una lista de variables y una lista de valores, de misma longitud, y crea un ambiente con ellas
  List x List -> amb
  <list-var> :: list
  <list-val> :: list
  <amb> :: <datatype>
...
Casos de prueba:
  ...
  (extend-env '(x y) '(5 3) (empty-env))
|#
(define extend-env
  (lambda (list-var list-val amb)
    (if (equal? list-var '())
        amb
        (extendido (car list-var) (car list-val) (extend-env (cdr list-var) (cdr list-val) amb)))
    )
  )

#|Función: apply-env
  Propósito: Busca una variable dentro de un ambiente
  List x symbol -> number / error
  <amb> :: vacio / extendido
  <var-buscar> :: <symbol>

Casos de prueba:
  (apply-env 'h (extend-env '(x y z) '(5 3 1) (empty-env)))
  (apply-env 'y (extend-env '(x y z) '(5 3 1) (empty-env)))
|#
(define apply-env
  (lambda (var-buscar amb)
    (cases ambiente amb
      (vacio ()  "Variable no encontrada")
      (extendido (var num amb) (if (equal? var-buscar var)
                                    num
                                    (apply-env var-buscar amb)))
      (else "Ambiente no valido")
      )
    )
  )
