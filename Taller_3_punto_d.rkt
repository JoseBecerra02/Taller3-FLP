...
#|Función: multiplicar
  Propósito: Multiplicar dos números usando unicamente add1 y sub1
  int x int -> int
  x :: int
  y :: int

declararRec(
@aux(@x, @y) = Si @y entonces add1(evaluar @aux(@x, sub1(@y)) finEval) sino @x finSI;
@multiplicar(@x, @y) = Si @y entonces evaluar @aux(evaluar @multiplicar(@x, sub1(@y)) finEval, @x) finEval sino 0 finSI)
...
#|Función: restar
  Propósito: restar dos números usando sub1
  int x int -> int
  x :: int
  y :: int

declararRec(@restar(@x, @y) = Si @y entonces evaluar @restar(sub1 (@x) , sub1 (@y)) finEval sino @x finSI)
